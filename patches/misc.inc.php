...
function redirect($target = null) {
	...
	$PORT="";
	if(array_key_exists('SERVER_PORT', $_SERVER) && $_SERVER["SERVER_PORT"]!='80' && $_SERVER["SERVER_PORT"]!='443') {
		$PORT=":".$_SERVER['SERVER_PORT'];
	}
	if(array_key_exists('HTTPS', $_SERVER) && $_SERVER["HTTPS"]=='on') {
		$url = "https://".@$_SERVER['SERVER_NAME'].$PORT.$target;
	} else {
		$url = "http://".@$_SERVER['SERVER_NAME'].$PORT.$target;
	}
	return $url;
}
